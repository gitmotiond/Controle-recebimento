...
export function loadData(): AppData {
  try {
    ...
    // Primeira execução: pré-popula com sugestões
    if (!raw && !seeded) {
...
export function addProduct(data: AppData, product: Omit<Product, "id" | "createdAt">): AppData {
  const newProduct: Product = {
    ...product,
    id: newId(),
    createdAt: new Date().toISOString(),
  };
  return { ...data, products: [...data.products, newProduct] };
}
...
export function updateConferente(
  ...
  return {
    ...data,
    conferentes: data.conferentes.map((c) => (c.id === item.id ? item : c)),
  };
}
export function deleteConferente(data: AppData, id: string): AppData {
  return { ...data, conferentes: data.conferentes.filter((c) => c.id !== id) };
}
...
export function updateColaborador(
  ...
  return {
    ...data,
    colaboradores: data.colaboradores.map((c) => (c.id === item.id ? item : c)),
  };
}
export function deleteColaborador(data: AppData, id: string): AppData {
  return { ...data, colaboradores: data.colaboradores.filter((c) => c.id !== id) };
}
...
export function updateLocal(
  ...
  return {
    ...data,
    locais: data.locais.map((c) => (c.id === item.id ? item : c)),
  };
}
export function deleteLocal(data: AppData, id: string): AppData {
  return { ...data, locais: data.locais.filter((c) => c.id !== id) };
}
...
export function upsertRecord(
  ...
  if (existing) {
    return {
      ...
      records: data.records.map((r) =>
        r.productId === record.productId && r.date === record.date
          ? {
              ...r,
  ...
  return { ...data, records: [...data.records, newRecord] };
}
...
export function calcFaltas(expected: number, received: number): number {
  const diff = expected - received;
  return diff > 0 ? diff : 0;
}

export function calcSobras(expected: number, received: number): number {
...
export function calcTotalUnits(qty: number, product: Product): number {
  if (!isContainerPackaging(product.packaging)) return qty;
  return qty * (product.unitsPerBox || 0);
}

// Converte unidades para caixas: qty (em un.) / unitsPerBox.
// Para acondicionamento "Unidade", 1 unidade = 1 unidade.
export function unitsToBoxes(units: number, product: Product): number {
  if (!isContainerPackaging(product.packaging)) return units;
  const upb = product.unitsPerBox || 1;
  return units / upb;
}

// Converte caixas para unidades.
export function boxesToUnits(boxes: number, product: Product): number {
  if (!isContainerPackaging(product.packaging)) return boxes;
